...
class UserDB:
    ...
    @classmethod
    async def find_many_or_none_by_id(cls, data_ids: list[int],
                                      session: AsyncSession = Depends(get_db)):
        query = select(db.User.id, db.User.name, db.User.surname).filter(db.User.id.in_(data_ids))
        result = await session.execute(query)
        users = [{"id": r[0], "name": r[1], "surname": r[2]} for r in result.all()]
        return users
